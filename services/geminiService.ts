...
export const extractContextGraphFactors = async (
  ...
  const keyMetrics: any = {
    ...
    schoolMaxRating,
    fireRisk: property.fireRiskScore ?? (property as any).climateRisk?.fire?.riskScore ?? null,
    floodRisk: property.floodRiskScore ?? (property as any).climateRisk?.flood?.riskScore ?? null,
    windRisk: property.windRiskScore ?? (property as any).climateRisk?.wind?.riskScore ?? null,
    heatRisk: property.heatRiskScore ?? (property as any).climateRisk?.heat?.riskScore ?? null,
