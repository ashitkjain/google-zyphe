...
async function seedBuyerInstructions() {
    ...
    const guideContent = {
        "introduction": "Discover properties using user stories written in natural language. Users can share their details on who they are, what their lifestyle and needs are, and then use AI to discover matching homes using nuanced insights, collected from comprehensive set of third party sources, google maps, places, gemini search grounding and visual AI.  Some of the cool features to try \n -Use the interactive AI Concierge chatbot to answer specific property questions in real-time.",
