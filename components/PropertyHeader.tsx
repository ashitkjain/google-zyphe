...
const PropertyHeader: React.FC<Props> = ({ data, isFavorited, onToggleFavorite, onRunAnalysis }) => {
  ...
  return (
    <div className="bg-white p-8 md:p-10 md:pb-2 rounded-t-[2.5rem] border-x border-t border-slate-100 shadow-sm space-y-8">
      ...
      {data.attribution && (data.attribution.listingAgentName || data.attribution.brokerageName) && data.images && data.images.length > 0 && (
        <div className="mt-8 bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100/80 flex flex-col sm:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-bottom-2 duration-700">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h4 className="text-sm font-black text-slate-900 leading-tight">
                {data.attribution.listingAgentName || 'Listing Agent'}
