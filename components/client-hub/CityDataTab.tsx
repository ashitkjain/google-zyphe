...
const CityDataTab: React.FC = () => {
    ...
    const handleAutoComplete = async (input: string) => {
        ...
        try {
            ...
            } else if (result.autocomplete) {
                // Fallback for some API versions
                setSuggestions(result.autocomplete);
            }
        } catch (err) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-end">
                <div className="lg:col-span-5 relative">
                    ...
                    {suggestions.length > 0 && (
                        <div className="absolute z-50 left-0 right-0 mt-2 bg-white border border-slate-100 rounded-xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 max-h-60 overflow-y-auto">
                            {suggestions.map((s, i) => (
                                <button
                                    key={i}
                                    onClick={() => selectSuggestion(s)}
                                    className="w-full px-5 py-3 text-left hover:bg-slate-50 flex items-center justify-between group transition-colors border-b border-slate-50 last:border-0"
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-sm font-bold text-slate-800">{s.city ? `${s.city}, ${s.state_code}` : s.area_type.toUpperCase()}</div>
                                            <div className="text-[10px] text-slate-400">{s.area_type} {s.postal_code}</div>
                                        </div>
                                    </div>
                                </button>
                            ))}
