    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex justify-center sm:justify-start">
        <div className="inline-flex bg-white p-1.5 rounded-2xl border border-gray-200 shadow-sm overflow-x-auto no-scrollbar max-w-full">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)} className={`flex items-center gap-3 px-6 py-3 rounded-xl font-black transition-all text-sm uppercase tracking-tight whitespace-nowrap ${activeTab === tab.id ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}`}>
          ...
          <section className="animate-in fade-in slide-in-from-bottom-2 duration-500">
            {!community_pulse ? <EmptyState section="Community Pulse" /> : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <PulseCard title="Safety & Concerns" data={community_pulse.safety_and_concerns} icon="fa-shield-halved" color="orange" />
                <PulseCard title="Schools & Family" data={community_pulse.schools_family_friendliness} icon="fa-children" color="blue" />
