 ...
 *
 * A sleek two-column dashboard-style overview page for property data.
 * Left column: Premium badge + property title + thematic data cards
 * Right column: Market estimate, AI score, mobility, education, nearby amenities
 *
 * Inspired by the Architectural Analyst reference UI.
 */
...
const SectionCard: React.FC<{ title: string; subtitle?: string; badge?: React.ReactNode; children: React.ReactNode; className?: string }> = ({
...
const MetricTile: React.FC<{ icon: string; iconBg: string; iconColor: string; label: string; value: string; sublabel?: string; valueColor?: string }> = ({
    ...
    return (
        <div className="flex flex-col xl:flex-row gap-4 py-4">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-4">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                    ...
                    <div className="px-5 py-4">
                        ...
                        <p className="text-[13px] text-slate-500 mt-0.5 font-medium">
                            {[data.address?.split(',').slice(1).join(',').trim()].filter(Boolean).join('')}
                        </p>
                        {/* Spec pills */}
                ...
                {(hasClimate || hasPollen || data.airQuality || hasNoise || data.drought || (data as any).historical_disasters) && (
                    <SectionCard
                        ...
                        <div className="space-y-4">
                                    ...
                                    {data.airQuality && (
                                        <div className="bg-slate-50 rounded-xl border border-slate-100 p-3">
                                            ...
                                            <div className="flex items-baseline gap-1.5">
                                                <span className={`text-[20px] font-black tracking-tight leading-none ${getAQIColor(data.airQuality.aqi)}`}>
                                                    {data.airQuality.aqi}
                                                </span>
                                                <span className="text-[10px] font-bold text-slate-400">AQI</span>
                                            </div>
                                            <div className={`text-[11px] font-black mt-0.5 ${getAQIColor(data.airQuality.aqi)}`}>
                                                {getAQILabel(data.airQuality.aqi)}
                                            </div>
                                        </div>
                                    )}
                                    {hasNoise && (
                                        <div className="bg-slate-50 rounded-xl border border-slate-100 p-3">
                                            ...
                                            <div className="flex items-baseline gap-1.5">
                                                <span className={`text-[20px] font-black tracking-tight leading-none ${data.noiseScore! >= 80 ? 'text-emerald-600' : data.noiseScore! >= 65 ? 'text-amber-600' : 'text-orange-600'}`}>
                                                    {data.noiseScore}
                                                </span>
                            ...
                            {data.drought && (data.drought.drought_level !== 'none' || data.seismicHazardZone) && (
                                <div className="bg-slate-800 rounded-xl p-3 space-y-1.5">
                                    ...
                                    {hasNoise && (
                                        <div className="flex items-center justify-between text-[12px]">
                                            <span className="text-slate-300 font-medium">Noise Pollution (Ldn)</span>
                ...
                {/* ── Solar Potential ────────────────────────────────── */}
                {hasSolar && (
                    <SectionCard title="Solar Potential" subtitle="Efficiency metrics">
                        <div className="grid grid-cols-3 gap-2">
                            <MetricTile
                                ...
                                label="Sunshine"
                                value={`${Math.round((solar!.maxSunshineHoursPerYear || 0) / 100) / 10}K`}
                                sublabel="hrs / yr"
                            ...
                            {solarPotential && (
                                <MetricTile
                                    ...
                                    label="Production"
                                    value={`${Math.round(solarPotential.annualKwh / 100) / 10}K`}
                                    sublabel="kWh / yr"
                            ...
                            {solar?.financialAnalysis?.cashPurchase?.paybackYears != null && (
                                <MetricTile
                                    ...
                                    label="Payback"
                                    value={`${Number(solar.financialAnalysis.cashPurchase.paybackYears).toFixed(1)}`}
                                    sublabel="years"
                            ...
                            {solar?.financialAnalysis?.cashPurchase?.savings?.savingsYear20 != null && (
                                <MetricTile
                                    ...
                                    label="20-Yr Savings"
                                    value={`$${Math.round(solar!.financialAnalysis!.cashPurchase!.savings!.savingsYear20 / 1000)}K`}
                            ...
                            {solarBench && (
                                <MetricTile
                                    ...
                                    label={`vs ${solarBench.benchmarkCity}`}
                                    value={`${solarBench.sunshinePctOfAvg}%`}
                                    sublabel={solarBench.sunshinePctOfAvg >= 100 ? 'Sun-Drenched' : 'Below Avg'}
                                    valueColor={solarBench.sunshinePctOfAvg >= 100 ? 'text-emerald-600' : 'text-amber-600'}
                                />
                            )}
                        </div>
                    </SectionCard>
                    ...
                    <SectionCard title="Connectivity">
                        <div className="space-y-2">
                            {data.broadband!.internetProviders.slice(0, 3).map((p, i) => {
                                ...
                                const speedLabel = p.maxDownloadMbps >= 1000
                                    ? `${(p.maxDownloadMbps / 1000).toFixed(1)} Gbps`
                                    : `${p.maxDownloadMbps} Mbps`;
                ...
                {hasCoords && (
                    <SectionCard
                        ...
                        subtitle="Seasonal shadow projections and daylight saturation"
                    >
                        <SeasonalSunCard
            ...
            <div className="xl:w-[300px] flex-shrink-0 flex flex-col gap-4">
                ...
                {/* Mobility Score */}
                {hasWalk && (
                    <SectionCard title="Mobility Score">
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={m.label}>
                                    ...
                                    {m.desc && (
                                        <div className="text-[10px] text-slate-400 font-medium mt-1 leading-snug">{m.desc}</div>
                                    )}
                                </div>
                            ))}
                        </div>
                ...
                {hasEV && (() => {
                    ...
                    return (
                        <SectionCard title="EV Charging">
                            <div className="grid grid-cols-3 gap-2 text-center">
                                {ev.totalCount != null && (
                                    <div>
                                        <div className="text-[20px] font-black text-slate-900">{ev.totalCount}</div>
                                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Stations</div>
                                    </div>
                                )}
                                {ev.closestDistanceMiles != null && (
                                    <div>
                                        <div className="text-[20px] font-black text-emerald-600">{ev.closestDistanceMiles.toFixed(1)}</div>
                                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">mi Away</div>
                                    </div>
                                )}
                                {ev.totalPorts != null && (
                                    <div>
                                        <div className="text-[20px] font-black text-slate-900">{ev.totalPorts}</div>
                                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Ports</div>
                                    </div>
                                )}
                            </div>
                            {ev.networks && ev.networks.length > 0 && (
                                <div className="flex flex-wrap gap-1.5 mt-3">
                                    {ev.networks.slice(0, 4).map((n: string, i: number) => (
                                        <span key={i} className="px-2 py-0.5 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded-full text-[9px] font-black uppercase tracking-wide">
                                            {n}
                                        </span>
                                    ))}
                                </div>
                ...
                {/* Schools */}
                {hasSchools && (
                    <SectionCard title="Education" badge={
                                ...
                                <div key={i} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-[10px] text-slate-400 font-medium">
                                            {school.distanceMiles ? `${school.distanceMiles.toFixed(1)} mi` : ''}
                        ...
                        <div className="space-y-2">
                            {((data as any).nearbyPlaces || []).slice(0, 5).map((place: any, i: number) => (
                                <div key={i} className="flex items-center gap-2.5">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className="text-[12px] font-bold text-slate-700 truncate block">{place.name || place}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {/* AI Analyst Summary (if available) */}
                    ...
                    <div className="bg-slate-900 rounded-2xl p-4">
                        ...
                        {onRunAnalysis && (
                            <button
                                ...
                                className="mt-3 w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-black uppercase tracking-widest rounded-xl transition-colors"
                            >
                                Full AI Audit
                            </button>
                        )}
