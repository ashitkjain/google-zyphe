...
export const neighborhoodNarrativeSchema = {
    type: "object",
    properties: {
        narrative: {
            type: "string",
            description: "A 2-3 sentence evocative narrative about the neighborhood and nearby amenities."
        }
    },
    required: ["narrative"]
};

export function getNeighborhoodNarrativePrompt(property: PropertyData, places?: any) {
    const address = property.address;
    const parks = (places?.parks || []).map((p: any) => `${p.name} (${(p.distanceMeters * 0.000621371).toFixed(1)} mi)`).slice(0, 3).join(", ");
    const dining = (places?.dining || []).map((p: any) => `${p.name} (${(p.distanceMeters * 0.000621371).toFixed(1)} mi)`).slice(0, 3).join(", ");
    const shopping = (places?.shopping || []).map((p: any) => `${p.name} (${(p.distanceMeters * 0.000621371).toFixed(1)} mi)`).slice(0, 3).join(", ");
