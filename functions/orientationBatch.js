...
function _buildOrientationPrompt(usesDualImage, address, description, streetBearing, streetSide) {
    ...
    if (usesDualImage) {
        return [
            ...
            `Step 2: Aerial front-wall identification. FACING CONVENTION (CRITICAL): the front wall faces TOWARD the street — if Calistoga Ln is on the SOUTH side, the front faces SOUTH (~180°), NOT north. Street to the south = face south. Street to the north = face north. Never invert. DIRECTION PRECISION: trace the driveway toward the street using BOTH axes — upper-left=NW(315°), upper-right=NE(45°), lower-left=SW(225°), lower-right=SE(135°). Only use N/E/S/W when movement is almost entirely in one axis.`,
